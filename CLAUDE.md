...
- Don't let anything on the e2e side name an unsuffixed `local/<name>` image — not a compose file, not a helper, not CI, not [docs/development/testing.md](docs/development/testing.md). E2e builds `local/<name>-test`; tags are host-wide and an e2e run must never overwrite what production deploys. Add new e2e image refs as constants in [portal/tests/e2e/\_helpers.py](portal/tests/e2e/_helpers.py).
