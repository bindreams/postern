...
| `SHADOWSOCKS_NETWORK`        | `shadowsocks`              | Docker bridge network the `ss-*` containers join; nginx attaches to the same one. `compose.yaml` names the real Docker network from this same variable, so the two cannot diverge. Give each deployment a distinct value if you run more than one Postern deployment on one host — unlike `postern.instance` container labelling, network isolation between co-located deployments is opt-in, not automatic. |
