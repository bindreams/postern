...
- Don't mutate a shipped migration to allow a new plugin value (or change any other historical schema). Migrations are immutable once published; add a new migration N+1 instead. To add a third plugin, add migration 3 that ALTERs the CHECK constraint via the table-rebuild pattern.
...
- Don't gate `ssl_mode_enabled` on `ech_enabled` — the redirect loop is independent of ECH.
