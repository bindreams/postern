...
async def list_connections(
    db: aiosqlite.Connection, *, user_id: str | None = None, enabled_only: bool = False
) -> list[Connection]:
...
async def set_connection_enabled(
    db: aiosqlite.Connection, connection_id: str, enabled: bool
) -> bool:
    cursor = await db.execute(
...
async def delete_connections_for_user(
...
async def verify_otp(
    db: aiosqlite.Connection, email: str, code_hash: str, max_attempts: int = 5
) -> bool:
