...
- **`local/` images: the tag is a version, the name carries the flavor.** `local/` is reserved for locally-built images with no online equivalent. E2e-built images therefore carry a `-test` suffix on the *name* (`local/nginx-test`), never on the tag (`local/nginx:e2e`) and never in a separate namespace (`local-e2e/nginx`) — a tag means "version" everywhere in this repo.
- **E2e images carry a `-test` suffix; production is unsuffixed.** Docker image tags are host-wide, so an e2e run that reused a production tag would silently replace the image a production `docker compose up -d` deploys. Everything the e2e suite builds or runs is `local/<name>-test`: the four e2e compose files' `image:` fields, the constants in [portal/tests/e2e/\_helpers.py](portal/tests/e2e/_helpers.py) (backing the one-off `docker run`/`docker build` calls in `_edge_helpers.py`, `test_tunnel.py`, and `test_mta_real.py`), and the two explicit `tags:` in [.github/workflows/test.yaml](.github/workflows/test.yaml) (the `docker/bake-action` steps inherit their tags from `image:` and need no separate edit). E2e portal services also set `SHADOWSOCKS_IMAGE=local/shadowsocks-server-test` — `Settings.shadowsocks_image` defaults to the production tag, and a portal pointed at it would adopt, recreate, and remove a co-located production stack's `ss-*` containers. Production still builds `local/shadowsocks-server` (and the other five images) unsuffixed by hand; the invariant is that *e2e* never writes an unsuffixed tag, not that nobody does. Pinned by [portal/tests/test_e2e_image_isolation.py](portal/tests/test_e2e_image_isolation.py), which derives the banned set from `compose*.yaml` + `Settings` and globs every path it scans, so a new file or a new production service is covered automatically.
